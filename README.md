...
- ساخت برنامه: انتخاب دانش‌آموز (جستجو + صفحه‌بندی) و سپس **دو حالت تاریخ**:
  «هفته تقویمی» (شنبه→جمعه) یا **بازه دلخواه** (تاریخ شروع تا پایان، حتی نیمه‌هفته)
...
**مدیر (فقط `ADMIN_IDS`)** — پنل مدیریت کامل و تماماً فارسی
- **🙋 درخواست‌های دسترسی:** هرکس ربات را بدون لینک دعوت باز کند در صف می‌نشیند
  (بدون ساخت حساب) و مدیر می‌تواند با یک کلیک به او نقش **مشاور** یا **دانش‌آموز**
  بدهد یا درخواست را رد کند؛ شخص بلافاصله پیام تأیید می‌گیرد
- **➕ افزودن مشاور** مستقیم با نام و شناسه تلگرام (بدون نیاز به Shell)
- **مدیریت مشاوران:** فهرست + جستجو، کارت کامل (دانش‌آموزان/برنامه‌ها/پیش‌نویس/ارسال‌ها/آخرین فعالیت)،
  ویرایش نام و شناسه تلگرام، تعلیق/فعال‌سازی، **حذف واقعی** با انتخاب سرنوشت دانش‌آموزان
  (انتقال به مشاور دیگر یا بدون مشاور)
- **مدیریت دانش‌آموزان:** فهرست + جستجو، کارت کامل، ویرایش، **تغییر مشاور**،
  مدیریت اتصال تلگرام (ساخت/ابطال لینک، قطع اتصال)، برنامه‌ها، تعلیق، **حذف واقعی**
- **مدیریت برنامه‌ها:** فهرست، کارت، دریافت تصویر/PDF، حذف با فایل‌ها
- **مدیریت فایل‌ها:** شمارش PNG/PDF، حجم، فایل‌های بدون رکورد و پاک‌سازی آن‌ها
- **پایگاه داده · آمار · گزارش فعالیت‌ها (فارسی) · سلامت سیستم · وضعیت ربات · تنظیمات**
- همه اعداد از کوئری زنده؛ عملیات خطرناک با تأیید دومرحله‌ای؛ حذف/تعلیق مدیر اصلی ممنوع
...
تمام منطق تاریخ در `app/domain/calendar.py` (`JalaliDate`) متمرکز است؛ هیچ هندلری
تقویم خودش را ندارد. دو حالت کاملاً جدا:

| حالت | ساختار | روز اول |
|---|---|---|
| هفته تقویمی | شنبه → جمعه (۷ روز) | همیشه شنبه (خودکار snap می‌شود) |
| بازه دلخواه | از تاریخ شروع تا پایان | **هر روزی که واقعاً در تقویم است** |

**روز هفته همیشه از تاریخ واقعی محاسبه می‌شود، نه از جایگاه در فهرست.**
نمونه واقعی — بازه ۲۶ تا ۲۹ مرداد ۱۴۰۵:

```
روز ۱ — دوشنبه   — ۲۶ مرداد ۱۴۰۵
روز ۲ — سه‌شنبه  — ۲۷ مرداد ۱۴۰۵
روز ۳ — چهارشنبه — ۲۸ مرداد ۱۴۰۵
روز ۴ — پنج‌شنبه — ۲۹ مرداد ۱۴۰۵
```

قوانین: `شروع ≤ پایان` · حداکثر طول بازه `MAX_PLAN_RANGE_DAYS` (پیش‌فرض ۷ روز،
هم‌اندازه ردیف‌های قالب رسمی) · ترتیب همیشه تقویمی · مبنا `Asia/Tehran`.

### تصمیم قالب (مستند)

برای بازه‌های کوتاه‌تر از یک هفته، **گزینه A** انتخاب شد: همان قالب رسمی هفت‌ردیفه
حفظ می‌شود و هر روز در ردیف *واقعی* خودش می‌نشیند؛ ردیف‌های خارج از بازه کاملاً
خالی می‌مانند. دلیل: صفر ریسک برای قالب pixel-accurate مؤسسه، بدون تغییر در
Renderer، و خروجی همچنان همان برگه رسمی رتبه لند است. (تست
`test_partial_range_renders_on_the_official_template` ردیف‌های خارج از بازه را
بایت‌به‌بایت با قالب خام مقایسه می‌کند.)
...
### حذف واقعی (Delete Integrity)

حذف در `app/services/deletion.py` و در **یک تراکنش** انجام می‌شود:
مجوز → جمع‌آوری داده‌های وابسته → حذف برنامه‌ها و نسخه‌ها → حذف روابط →
حذف رکورد کاربر → حذف فایل‌ها (فقط داخل `STORAGE_ROOT`) → Audit → Commit.
هر خطا باعث **Rollback کامل** می‌شود و پیام موفقیت داده نمی‌شود.
پیش از حذف، اثر دقیق آن (تعداد برنامه‌ها، نسخه‌ها و فایل‌ها) نمایش داده می‌شود و
تأیید **دو مرحله‌ای** لازم است. حذف مشاور هرگز دانش‌آموزان را کورکورانه نابود نمی‌کند.

### ورود افراد جدید (Access Requests)

هیچ‌کس با `/start` حساب نمی‌گیرد. یک بازدیدکنندهٔ ناشناس فقط به‌عنوان **درخواست
دسترسی** ثبت می‌شود (جدول `access_requests`: نام، شناسه، تعداد مراجعه، وضعیت) و
پیام «درخواست شما برای مدیر ارسال شد» می‌بیند. مدیر در پنل:

```
🛠 پنل مدیریت → 🙋 درخواست‌های دسترسی → انتخاب شخص
   → 👨‍🏫 تأیید به‌عنوان مشاور
   → 👨‍🎓 تأیید به‌عنوان دانش‌آموز (با انتخاب مشاور)
   → 🚫 رد درخواست
```

اعطای نقش **فقط** از این مسیر (یا «➕ افزودن مشاور») ممکن است؛ لینک دعوت همچنان
هرگز نقشی را تغییر نمی‌دهد. هر تصمیم در گزارش فعالیت‌ها ثبت و به خود شخص اطلاع
داده می‌شود.

### وضعیت اتصال در پروفایل دانش‌آموز

پروفایل هر دانش‌آموز، وضعیت اتصال و **لینک دعوت واقعی** را مستقیم نشان می‌دهد:
🟢 متصل · 🟡 لینک صادر شده · 🔴 لینک منقضی · ⚪️ بدون لینک — به‌همراه تاریخ صدور و
انقضا، و دکمه‌های «کپی لینک»، «ارسال لینک» و «ساخت لینک جدید» (با هشدار قبل از
باطل کردن لینک فعال). لینک مصرف‌شده هرگز به‌عنوان فعال نمایش داده نمی‌شود.
